...
const HIDDEN_SERVICES = [
  ...
  'artificial.adapter_common.InternalActions',
];
...
export function credsConstructor(md: grpc.Metadata, ssl = true): () => grpc.ChannelCredentials {
  return () => {
    if (ssl) {
...
async function extractServiceClientConstructor(
  client: GrpcReflection,
  service: string,
  timeoutMs: number,
): Promise<[grpc.ServiceClientConstructor, Array<string>]> {
...
export async function getRemoteScope(
  address: string,
  lab: string,
  token: string,
  ssl = true,
 ...
 */
export async function getAdapterClients(
  address: string,
  md: grpc.Metadata,
  ssl = true,
  timeoutMs = 5000,
): Promise<Map<string, AdapterClient>> {
  // Connect with grpc server reflection
  const creds = credsConstructor(md, ssl);
  ...
  for (const service of filteredServices) {
    ...
    // Add to clients
    const grpcClient = new clientConstructor(address, creds(), channelOptions());
...
export interface LabmanagerClient {
  getConnections(
    ...
    callback: (error: grpc.ServiceError | null, response: GetConnectionsResponse) => void,
  ): void;
  getConnections(
    request: GetConnectionsRequest,
...
export async function getLabmanagerClient(
  address: string,
  md: grpc.Metadata,
  ssl = true,
  _timeoutMs = 5000,
): Promise<LabmanagerClient> {
  const creds = credsConstructor(md, ssl);
