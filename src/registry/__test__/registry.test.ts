...
describe('testregistry', function () {
  test('test list tags - normal', async function () {
    ...
    const r = Registry.createFromGithub(
      'https://github.com/artificialinc/artificial-ADAPTER-template.git',
      'user',
      'token',
    );
    ...
    expect(tags).toStrictEqual([
      'ghcr.io/artificialinc/artificial-adapter-template:aidan-6',
      'ghcr.io/artificialinc/artificial-adapter-template:aidan-5',
    ]);
  });
  ...
  test('test list tags ssh', async function () {
    ...
    const r = Registry.createFromGithub(
      'git@github.com:artificialinc/artificial-adapter-template.git',
      'user',
      'token',
    );
    const tags = await r.listTags();
  ...
  test('test list tags non-github', async function () {
    jest.spyOn(registryClient, 'getTags').mockResolvedValue(['aidan-5']);

    expect(() => {
      Registry.createFromGithub('https://gitlab.com/artificialinc/artificial-adapter-template.git', 'user', 'token');
    }).toThrow('Invalid github url: https://gitlab.com/artificialinc/artificial-adapter-template.git');
  });
